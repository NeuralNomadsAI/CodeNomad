...
- Server and UI pin the official `@opencode/client@2.0.11` together; bundled plugins pin `@opencode/plugin@2.0.11`. The canonical server-info client is `server.info()`, adapted to the authenticated discovered route. Review official V2 docs, installed declarations, generated wire paths and native integration tests when upgrading. The runtime CLI is managed independently and startup has no exact version gate.
...
- There is no legacy `packages/opencode-plugin/`. Do not restore the V1 compatibility runtime or add general plugin extension points. The narrow integrations are the bundled `codenomad.automation` plugin, project-local native session coordination (`codenomad.missions`), and bundled session-pruning RPC; see `dev-docs/DEVELOPER_MODE.md`, `dev-docs/BROWSER_AUTOMATION.md`, `dev-docs/MISSIONS.md` and `dev-docs/SESSION_PRUNING_RPC.md`. All automation tools follow backend presence without a Developer Mode gate, sharing the authenticated native transport and execution-time session/window fences.
...
- Inspect installed declarations under `node_modules/@opencode/client/dist/promise/`; generated names are the source of truth.
- Public locations are directory-only `LocationPublicRef`; `location.get().project` owns project metadata. Internal/event `LocationRef.workspaceID` is not a public request selector. Validate directory ownership at the server boundary.
...
| Reintroducing the V1 `packages/opencode-plugin` or general server plugin/background-process paths | Native OpenCode APIs; narrow reviewed automation/browser-preview, Missions and session-pruning integrations |
