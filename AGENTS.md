...
- V2 plugin activation controls use the square persistent switch in `styles/components/plugin-controls.css`; scope selection lives separately in `plugin-control-scope.css`. Keep runtime inventory distinct from configured sources/rules and require an explicit Global or Project scope before writing an ordered rule.
