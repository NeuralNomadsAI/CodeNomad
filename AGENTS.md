...
- OpenCode settings show a compact runtime summary with update availability and a direct install/update action; additional management actions live in the shared setup dialog, also mounted by the native preferences window. Keep diagnostics, executable selection and configuration reload in explicit disclosure sections styled in `styles/components/opencode-setup.css`.
...
- Timeline geometry uses the cached structural index independently of transcript/excerpt loading. Hover/focus previews render bounded Markdown, prefetch visible-nearby excerpts and prioritize the hovered marker through `stores/timeline-previews.ts`; never mount whole message/tool cards or fetch transcript windows for hover. Keep the viewport-bounded surface in `styles/messaging/timeline-preview.css`.
...
- Worktree branch/HEAD annotations use one NUL-delimited Git worktree snapshot, intersected with native inventory and verified Git common-directory/backlink identities; avoid per-checkout Git process fans. A pending selector move displays its requested target with a busy state until native reconciliation settles.
