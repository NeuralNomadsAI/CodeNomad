...
- Full-history search/counts use the bundled pruning plugin's bounded queries, outside the transcript store; explicit result previews fetch one native message only. Search/results/progress styles live in `styles/components/history-search.css`. See `dev-docs/SESSION_HISTORY_QUERIES.md` for scope, snapshot and cleanup semantics.
