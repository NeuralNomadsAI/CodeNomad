...
| --- | --- | --- |
| Session outline | Structural indexes independent of transcript and excerpts; up to 16 snapshots / 200k entries (one larger active index allowed). Unchanged return makes no read; changed native messages refresh 32 tail rows plus arrivals. Destructive mutation, generation and undo boundary fence reuse. Paused scans retain cursors. | `packages/ui/src/stores/session-outline.ts` |
