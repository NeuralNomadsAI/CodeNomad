...
23. [关键文件索引](#23-关键文件索引)
24. [超限文件告警](#24-超限文件告警)
25. [Agent 工作流感知机制](#25-agent-工作流感知机制)
...
| **命令面板** | 快捷命令操作 |
| **文件浏览器** | 内置文件系统浏览与模糊搜索 |
| **多类型预览器** | Markdown / 图片 / 音频 / 视频 / PDF / 代码编辑器预览 |
...
| Package | 文件数 | 总行数 | 语言 |
|---------|--------|--------|------|
| **server** | 63 | ~10,916 | TypeScript |
| **ui** | ~410 | ~66,790 | TSX/TS/CSS |
| **electron-app** | 9 | ~2,094 | TypeScript |
| **tauri-app** | 2 | ~1,916 | Rust |
| **opencode-config** | 5 | ~674 | TypeScript |
...
│       │   ├── http-server.ts   #   Fastify 核心 (1154行)
│       │   ├── tls.ts           #   TLS 证书管理
│       │   ├── network-addresses.ts # LAN IP 解析
│       │   └── routes/          #   模块化路由 (14 files + 文件操作路由)
│       │       ├── workspaces.ts
│       │       ├── worktrees.ts
│       │       ├── settings.ts
│       │       ├── storage.ts
...
│       │   ├── manager.ts       #   WorkspaceManager (533行)
...
│       │   ├── browser.ts       #   FileSystemBrowser (+deleteFile, +uploadFile)
...
│       ├── lib/                 # 核心库 (42 entries)
│       │   ├── api-client.ts    #   HTTP API 客户端 (+upload/download/deleteWorkspaceFile)
│       │   ├── file-types.ts    #   文件类型检测/分类
...
│       │   ├── i18n/            #   国际化系统
│       │   │   ├── index.tsx    #     i18n 核心
│       │   │   └── messages/    #     7 种语言, 每种 18 个消息分片 (+fileViewer)
│       │   ├── native/          #   原生功能抽象
│       │   │   ├── native-functions.ts
...
│           ├── messaging/       #   消息/输入/工具调用样式 (11 files)
│           └── panels/          #   面板/布局/标签样式 (7 files, +file-viewers.css)
│
...
**无自动重启机制**: `handleProcessExit()` (manager.ts:511-532) 检测到非零退出码时设置 `status: "error"` 并发布 `workspace.error` 事件，workspace 保持在 error 状态。不会尝试自动重启。

**恢复方式**: 用户需手动停止（delete）崩溃的工作空间，然后创建新实例。

**原因**: OpenCode 实例崩溃通常意味着严重的状态异常（如内存损坏、死锁），自动重启可能产生相同错误循环。当前设计选择让用户介入判断。
...
| `/api/workspaces/:id/files/content` | GET/PUT/DELETE | 读取/写入/删除文件内容 (worktree 感知) |
| `/api/workspaces/:id/files/upload` | POST | 上传文件 (multipart, 100MB 限制, worktree 感知) |
| `/api/workspaces/:id/files/download` | GET | 下载文件 (支持 Range 分块, worktree 感知) |
...
**OpenCode → Server 桥接事件** (通过 SSE 消费，由 EventBus 转发到 UI):

| 事件类型 | 来源 | 文件 |
|---------|---------|------|
| `message.updated` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `message.part.delta` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `message.part.updated` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `message.removed` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `session.status` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `session.idle` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `session.compacted` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `session.diff` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `permission.asked` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `permission.replied` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `question.asked` | OpenCode SSE → InstanceEventBridge | instance-events.ts |
| `question.replied` | OpenCode SSE → InstanceEventBridge | instance-events.ts |

---

## 7. Server 模块详解

> Server 包含 63 个 TypeScript 源文件，总计约 10,916 行代码。

### 7.1 HTTP 服务器 (`server/`)

**核心文件**: `http-server.ts` (1154行)
...
| 模块 | 行数 | 职责 |
|------|------|------|
| `workspaces.ts` | 279 | 工作空间 CRUD + 文件上传/下载/删除 |
...
| `background-processes.ts` | 103 | 后台进程管理 |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `manager.ts` | 533 | `WorkspaceManager` — 创建/启动/停止/删除工作空间 |
...
| 文件 | 行数 | 职责 |
|------|------|------|
| `browser.ts` | 407 | `FileSystemBrowser` — 目录浏览、文件读写/上传/删除 (受限/开放模式)，Windows 驱动器枚举 |
...
| `types/fastify-multipart.d.ts (1行) | fastify-multipart 类型声明 |
...
> UI 包含约 410 个源文件，总计约 66,790 行代码。
...
│           │   └── RightPanel (1091行)  # 右侧面板 (+二进制, +i18n, +tab重获取)
│           │       ├── ChangesTab       # 文件变更
│           │       ├── GitChangesTab    # Git 变更
│           │       ├── FilesTab         # 文件浏览 (tabs/FilesTab.tsx, 320行)
│           │       └── StatusTab        # 状态信息
│           │       └── hooks/           # 文件操作 hooks
│           │           └── useFileOperations.ts  # 上传/下载/删除
│           └── MessageTimeline (1127行) # 消息时间线
│               └── MessageSection (1386行) # 消息区域
│                   └── MessageBlock (1629行) # 消息块 (最大组件)
...
| 模块 | 行数 | 职责 |
|------|------|------|
| `api-client.ts` | 614 | HTTP 客户端 — Fetch 封装所有 Server API (+upload/download/deleteWorkspaceFile) |
...
| `file-types.ts` | 74 | 文件类型检测与 MIME 分类 |
...
- 每种语言有 **18 个消息分片文件**: advancedSettings, app, commands, dialogs, filesystem, folderSelection, index, instance, loadingScreen, logs, markdown, messaging, remoteAccess, session, settings, time, toolCall, fileViewer
...
3. 聚合入口:
   ...
   ├── messaging/  (11 files) — 消息/输入/工具调用样式
   └── panels/     (7 files, +file-viewers.css)  — 面板/布局/标签样式
```
...
**5 个源文件**, ~674 行。依赖: `@opencode-ai/plugin` (v1.4.3)
...
**阶段 2: Agent 流式响应**
```
OpenCode SSE → /global/event
    │
    ├─ message.part.delta (流式文本增量)
    │   ▼
    │   applyPartDelta() → bumpSessionRevision() → UI 增量渲染
    │
    ├─ message.part.updated (tool call 状态变化)
    │   ▼
    │   applyPartUpdate() → bumpSessionRevision() → 工具调用 UI 更新
    │
    └─ session.status (type: "busy")
        ▼
        ensureSessionStatus("working") → 侧边栏显示"工作中"状态
```

**阶段 3: 权限/问题请求（Agent 需要用户输入时）**
```
permission.asked / question.asked
    │
    ▼
session-events.ts → handlePermissionUpdated() / handleQuestionAsked()
    │
    ▼
instances.ts → addPermissionToQueue() → setSessionPendingPermission(true)
    │
    ▼
ToolCall 组件检测 pendingPermission → 渲染 PermissionToolBlock
    │
    ▼
用户选择: Allow Once / Allow Always / Reject
    │
    ▼
sendPermissionResponse() → SDK client.permission.reply({ requestID, reply })
    │
    ▼
OpenCode 执行操作，继续工作
    │
    ▼
permission.replied SSE → removePermissionFromQueue() → 重新计算 ActiveInterruption
```

**阶段 4: 消息完成**
```
message.updated (info.time.end 存在)
    │
    ▼
status: "complete", bumpRevision: true
    │
    ▼
sessionRevisions[sessionId]++ → UI 组件检测 revision 变化 → 完整重渲染
```

**阶段 5: Agent 空闲（一轮工作结束）**
```
session.idle
    │
    ▼
ensureSessionStatus("idle") → 侧边栏显示"空闲"
    │
    ▼
UI 更新: 侧边栏 diff、文件列表、token 统计
```
...
| 触发事件 | 目标状态 | 文件:行号 |
|----------|----------|-----------|
| `session.status` (type: "busy") | `working` | session-events.ts:561-588 |
| `session.idle` | `idle` | session-events.ts:546-559 |
| `message.part.updated` (compaction part) | `compacting` | session-events.ts:329-330 |
| `session.compacted` | `working` | session-events.ts:591-621 |
| `session.status` (type: "retry") | `working` (设置 retry 状态) | session-events.ts:569-587 |

**关键规则**: 从 `compacting` 状态不能降级到其他状态（session.ts:157-159 守卫），确保上下文压缩不被打断。
...
- **Revision 驱动 UI 响应**: `sessionRevisions` 是每会话单调递增计数器（instance-store.ts:317-324），每次以下操作都会 `bumpSessionRevision()`:
  - `upsertMessage()`（新消息或消息状态变更）
  - `applyPartUpdate()`（part 更新，含 tool call 状态变化）
  - `applyPartDelta()`（流式文本增量）
  - `removeMessage()` / `removeMessagePart()`
  
  UI 组件通过 `createMemo(() => getSessionRevision(sessionId))` 订阅此计数器，值变化时触发 Solid.js 响应式重渲染。这是 UI 感知 Agent 工作进展的核心机制。
- **Revision 重置**: `clearSession()` 时会重置该会话的 revision 计数器
...
CodeNomad 通过 SSE 事件流实现与 OpenCode 实例的双向通信，完整感知 Agent 的工作生命周期。本节描述 Agent 从接收用户 prompt 到完成一轮工作的完整流程。

### 25.1 任务完整生命周期

```
┌─ 阶段 1: 发送 Prompt ──────────────────────────────────────────────┐
│  用户输入 → sendMessage() → HTTP POST → Server 代理 → OpenCode     │
│  文件: session-api.ts → http-server.ts → OpenCode serve            │
└────────────────────────────┬───────────────────────────────────────┘
                             │
┌─ 阶段 2: Agent 流式处理 ──────────────────────────────────────────┐
│  OpenCode SSE /global/event → InstanceEventBridge → EventBus       │
│    ↓                                                               │
│    message.part.delta  ──→ applyPartDelta() ──→ bumpRevision      │
│    message.part.updated ──→ applyPartUpdate() ──→ bumpRevision    │
│    session.status(busy) ──→ ensureSessionStatus("working")         │
│  文件: instance-events.ts → session-events.ts → instance-store.ts  │
└────────────────────────────┬───────────────────────────────────────┘
                             │
┌─ 阶段 3: 权限/问题请求（分支）────────────────────────────────────┐
│  permission.asked ──→ 加入权限队列 ──→ 用户响应 ──→ SDK reply ──→│
│  question.asked   ──→ 加入问题队列 ──→ 用户回答 ──→ SDK reply ──→│
│  文件: session-events.ts → instances.ts → tool-call.tsx           │
└────────────────────────────┬───────────────────────────────────────┘
                             │
┌─ 阶段 4: 消息完成 ─────────────────────────────────────────────────┐
│  message.updated (info.time.end) ──→ status: "complete"           │
│  ──→ bumpRevision ──→ UI 重渲染                                    │
│  文件: session-events.ts:372-427                                   │
└────────────────────────────┬───────────────────────────────────────┘
                             │
┌─ 阶段 5: Session Idle（一轮工作结束）─────────────────────────────┐
│  session.idle ──→ ensureSessionStatus("idle")                     │
│  ──→ UI 更新: 侧边栏 diff、文件列表、token 统计                     │
│  文件: session-events.ts:546-559                                  │
└────────────────────────────────────────────────────────────────────┘
```

### 25.2 Session 状态机

Session 状态在 `types/session.ts` 定义为 `"idle" | "working" | "compacting"`。
...
```
  ...
  └──┬───┘                         └───────▲───────┘
     ...
     └───────────────────────────────── 保持 working，设置 retry 状态
```

**完整转换表**:

| 当前状态 | 触发事件 | 目标状态 | 文件:行号 |
|----------|----------|----------|-----------|
| 任意 | `session.status` (type: "busy") | `working` | session-events.ts:561-588 |
| 任意 | `session.idle` | `idle` | session-events.ts:546-559 |
| 任意 | `message.part.updated` (compaction) | `compacting` | session-events.ts:329-330 |
| `compacting` | `session.compacted` | `working` | session-events.ts:591-621 |
| `compacting` | 任何其他状态事件 | **保持 compacting** (守卫) | session.ts:157-159 |

**Guard 规则** (session.ts:157-159):
```typescript
if (current === "compacting" && status !== "compacting") {
  return false  // 不允许从 compacting 降级
}
```

**Retry 状态**: 当 `session.status` 为 `type: "retry"` 时，Session 状态保持 `"working"`，同时设置 `retry: { attempt, message, next }`。UI 显示 toast 倒计时通知（session-events.ts:569-587）。`next` 为下次重试的时间戳。

### 25.3 Message 完成检测

Agent 一轮工作的完成不是由单一信号决定，而是多层信号综合判断：

**单条消息完成** — `message.updated` 事件 (session-events.ts:372-427):
```typescript
...
```
- `info.time.end` 存在 → 消息完成
- `info.error` 存在 → `status: "error"`
- 必须调用 `bumpRevision: true` → 递增 sessionRevisions → UI 重渲染

**整体会话空闲** — `session.idle` 事件 (session-events.ts:546-559):
- 表示 OpenCode 已完成所有消息处理，进入等待状态
- UI 据此更新侧边栏状态、刷新 token 统计

**Revision 响应式机制** (instance-store.ts:317-324):
```typescript
...
UI 消费方式:
```typescript
const revision = createMemo(() => getSessionRevision(sessionId))
// revision 变化触发组件重渲染
```

### 25.4 权限/问题请求-响应闭环

完整 10 步流程（从 OpenCode 请求到用户响应再到继续）:

```
1. Agent 执行需权限操作 (如 edit file)
2. OpenCode SSE: permission.asked
3. session-events.ts: handlePermissionUpdated()
4. instances.ts: addPermissionToQueue()
   ├── 按创建时间排序
   ├── 计算 ActiveInterruption（最老的权限优先）
   └── setSessionPendingPermission(instanceId, sessionId, true)
5. ToolCall 组件检测 pendingPermission → 渲染 PermissionToolBlock
   ├── 显示 diff 内容
   ├── 按钮: "Allow Once" / "Allow Always" / "Reject"
   └── 快捷键: Enter=once, A=always, D=deny
6. 用户点击/按键 → instances.ts: sendPermissionResponse()
7. SDK: client.permission.reply({ requestID, reply })
8. OpenCode 执行操作，继续工作
9. OpenCode SSE: permission.replied
10. session-events.ts: handlePermissionReplied()
    ├── removePermissionFromQueue()
    └── 重新计算 ActiveInterruption
```

**问题请求**流程类似，使用 `question.asked` → `question.replied`/`question.rejected`。

**优先级机制** (instances.ts:640-653):
- 同一时间只有一个"活跃中断"（ActiveInterruption）
- 权限优先于问题（如果权限创建时间 ≤ 问题创建时间）
- 队列头部是最活跃的中断

### 25.5 错误与恢复机制

**崩溃不自动重启** (manager.ts:511-532):
- `handleProcessExit()` 检测非零退出码 → `status: "error"` → 发布 `workspace.error`
- **无 auto-restart 逻辑** — workspace 保持在 error 状态
- 恢复方式: 用户手动 delete 崩溃 workspace → 创建新实例
- 设计原因: 崩溃通常意味着严重状态异常，自动重启可能产生相同错误循环

**SSE 断线重连**:

| 层级 | 策略 | 文件 |
|------|------|------|
| Server 端 | 1s 常数延迟重试，无指数退避 | instance-events.ts:86-93 |
| UI 端 | 指数退避: `RETRY_BASE_DELAY` × 2，上限 `RETRY_MAX_DELAY` | server-events.ts:22-63 |

**实例重水合** (instances.ts:308-334):
- `rehydrateInstance()` 清除缓存、命令、元数据、权限/问题队列
- 重新加载 worktrees、sessions、agents、providers、config、commands
- 从服务器同步待处理权限/问题
- 触发时机: dispose 事件、连接恢复后

### 25.6 文件变更追踪

CodeNomad 使用**两种方式**追踪 Agent 对文件的修改:

| 方式 | 机制 | 用途 | 文件 |
|------|------|------|------|
| `session.diff` SSE 事件 | 事件驱动，接收 `FileDiff[]` 数组 | Changes 标签页显示 agent 修改 | session-events.ts:521-544 |
| 文件浏览器刷新 | 手动（用户点击 refresh 按钮） | 浏览 worktree 文件系统 | RightPanel.tsx:820-866 |

**⚠️ 已知限制**: 文件浏览器不会自动刷新。Agent 修改文件后，用户必须手动点击 refresh 按钮才能在文件列表中看到变化。Changes 标签页通过 `session.diff` SSE 事件可以实时看到变更，但文件浏览器（Files 标签页）不会自动同步。
...
| **HTTP 服务器** | `server/http-server.ts` | 1154 | `createHttpServer()`, `proxyWorkspaceRequest()` |
| **TLS 管理** | `server/tls.ts` | 283 | TLS 证书解析/自动生成 |
| **网络地址** | `server/network-addresses.ts` | 128 | LAN IP 解析 |
| **工作空间路由** | `server/routes/workspaces.ts` | 279 | CRUD + upload/download/delete |
| **工作空间管理** | `workspaces/manager.ts` | 533 | `WorkspaceManager.create()`, `delete()`, `shutdown()` |
...
| **文件浏览** | `filesystem/browser.ts` | 407 | `FileSystemBrowser` (+deleteFile, +uploadFile) |
...
| **右侧面板** | `components/instance/shell/right-panel/RightPanel.tsx` | 1091 | `RightPanel` |
| **消息块** | `components/message-block.tsx` | 1629 | `MessageBlock` |
| **消息区域** | `components/message-section.tsx` | 1386 | `MessageSection` |
| **消息时间线** | `components/message-timeline.tsx` | 1127 | `MessageTimeline` |
...
| **API 客户端** | `lib/api-client.ts` | 614 | `serverApi`, `connectEvents()` |
...
| **文件类型** | `lib/file-types.ts` | 74 | 文件类型检测与 MIME 分类 |
| **文件操作 Hooks** | `components/instance/shell/right-panel/hooks/useFileOperations.ts` | — | 上传/下载/删除 |
| **文件标签** | `components/instance/shell/right-panel/tabs/FilesTab.tsx` | 320 | 文件浏览 |
...
### CSS (packages/ui/src/styles/)

| 功能 | 文件路径 | 行数 |
|------|----------|------|
| **右侧面板** | `panels/right-panel.css` | 603 |
| **文件预览** | `messaging/file-viewers.css` | 80 |
...
| 文件 | 行数 | 超出 |
|------|------|------|
| `packages/ui/src/components/message-block.tsx` | 1629 | +829 |
| `packages/ui/src/stores/message-v2/instance-store.ts` | 1241 | +441 |
| `packages/ui/src/components/message-section.tsx` | 1386 | +586 |
| `packages/server/src/server/http-server.ts` | 1154 | +354 |
...
| `packages/ui/src/components/instance/shell/right-panel/RightPanel.tsx` | 1091 | +291 |
| `packages/ui/src/components/instance/instance-shell2.tsx` | 952 | +152 |
| `packages/ui/src/components/tool-call.tsx` | 941 | +141 |
| `packages/ui/src/components/message-timeline.tsx` | 1127 | +327 |
| `packages/ui/src/stores/preferences.tsx` | 885 | +85 |
...
| `packages/ui/src/styles/panels/right-panel.css` | 603 | CSS |
| `packages/server/src/workspaces/manager.ts` | 533 | Manager |
| `packages/ui/src/lib/api-client.ts` | 614 | Lib |
...
| `packages/server/src/server/routes/workspaces.ts` | 279 | Routes |
| `packages/server/src/filesystem/browser.ts` | 407 | Lib |
...
*文档更新完毕 — 2026-04-12 (第三轮补充: Agent 工作流感知机制、Session 状态机、Revision 响应式、权限/问题闭环、错误恢复、文件变更追踪、§11.1 完整工作流、§15.2 状态机、§15.4 sessionRevisions)*
