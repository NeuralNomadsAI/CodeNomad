...
test("web settings use explicit scopes, native keys, no mutation replay and fence late writes", async () => {
  ...
  try {
    ...
    await page.getByText("Search in this project: Disabled", { exact: true }).waitFor()
    ...
    assert.equal(await page.getByLabel("Override for this project", { exact: true }).inputValue(), "off")
    await page.getByText("From environment: ALPHA_API_KEY (read-only)", { exact: true }).waitFor()
    ...
    await page.getByText("Search in this project: Alpha", { exact: true }).waitFor()
