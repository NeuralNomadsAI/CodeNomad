...
test("web settings use explicit scopes, native keys, no mutation replay and fence late writes", async () => {
  ...
  try {
    ...
    await page.getByLabel("Override for this project", { exact: true }).selectOption("default")
    ...
    assert.equal(await page.getByLabel("Override for this project", { exact: true }).inputValue(), "default")
    await page.evaluate(() => (window as any).fixture.setActive(false))
    await page.getByLabel("Override for this project", { exact: true }).waitFor({ state: "detached" })
    await page.goto(url)
    await page.getByLabel("Default for all projects", { exact: true }).waitFor()
    assert.equal(await page.getByLabel("Override for this project", { exact: true }).count(), 0)
    assert.equal(await page.locator(".settings-card-subtitle").count(), 0)
    ...
    assert.equal(await page.getByLabel("Web search provider", { exact: true }).inputValue(), "alpha")
    assert.equal(await page.getByLabel("Default for all projects", { exact: true }).inputValue(), "provider:alpha")
    await page.getByTitle("From environment: ALPHA_API_KEY (read-only)", { exact: true }).waitFor()
    ...
    await page.getByLabel("Default for all projects", { exact: true }).selectOption("provider:random")
    await page.evaluate(() => (window as any).fixture.setDirectory("/b"))
    await page.waitForFunction(() => (document.querySelector('select[aria-label="Default for all projects"]') as HTMLSelectElement)?.value === "provider:alpha")
