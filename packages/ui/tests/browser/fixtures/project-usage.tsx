...
const [directory, setDirectory] = createSignal("/repo"), [visible, setVisible] = createSignal(true)
let fail = false, defer = false, release: (() => void) | undefined
const queries: any[] = []
serverApi.getProjectUsage = async (_id, query) => {
  queries.push(query)
  if (defer) { defer = false; await new Promise<void>(resolve => { release = resolve }) }
  if (fail) { fail = false; throw new Error("fixture failed") }
  const steps = query.directory === "/repo" ? 42 : 7
  const tokens = { input: 12345, output: 987, reasoning: 100, cache: { read: 6789, write: 456 } }
  return { project: "fixture", directory: query.directory, stats: { range: { from: query.from, to: query.to }, sessions: 3, subagents: 2, prompts: 10, steps,
    ...
    activity: [{ date: "2026-09-25", steps: 20 }, { date: "2026-09-26", steps: steps - 20 }],
  } }
}
...
render(() => <ConfigProvider><I18nProvider><ThemeProvider><Show when={visible()}>
  {new URLSearchParams(location.search).has("parent")
    ? <SettingsScreen standalone providerContext={{ instanceId: "usage", location: { directory: directory() } }} />
    : <main style={{ width: "min(760px, 100%)" }}><UsageSettingsSection instanceId="usage" location={{ directory: directory() }} /></main>}
</Show></ThemeProvider></I18nProvider></ConfigProvider>, document.getElementById("root")!)
;(window as any).fixture = { queries, setDirectory, setVisible, fail: () => { fail = true }, defer: () => { defer = true }, release: () => release?.(),
