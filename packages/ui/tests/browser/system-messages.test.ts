...
test("popup controls preserve content, synchronize Chat settings, and survive reopening", async () => open(async page => {
  ...
  const row = page.getByRole("group", { name: "System messages", exact: true })
  await row.getByRole("button", { name: "Show System messages", exact: true }).click()
  ...
  await row.getByRole("button", { name: "Expand System messages", exact: true }).click()
  ...
  assert.equal(await page.locator('#chat-settings .settings-expansion-row').filter({ hasText: "System messages" }).locator(".selector-trigger-primary").textContent(), "Expanded")
  await page.reload()
  await page.waitForFunction(() => Boolean((window as any).fixture))
  await page.locator(`${system} pre`).waitFor()
  await page.getByRole("button", { name: "Message content", exact: true }).click()
  await row.getByRole("button", { name: "Hide System messages", exact: true }).click()
...
test("visible live notices render as System and obey local and global disclosure", async () => open(async page => {
  ...
  await page.getByRole("button", { name: "Message content", exact: true }).click()
  await row.getByRole("button", { name: "Expand System messages", exact: true }).click()
  await row.getByRole("button", { name: "Collapse System messages", exact: true }).click()
  assert.equal(await page.locator(`${system} pre`).count(), 0)
}))
