...
test("MCP source controls persist three states, reconcile failures and fence late location responses", async () => {
  ...
  try {
    ...
    const inactiveReads = await page.evaluate(() => (window as any).fixture.reads())
    await page.evaluate(() => { (window as any).fixture.refresh(); (window as any).fixture.release() })
    assert.equal(await page.evaluate(() => (window as any).fixture.reads()), inactiveReads)
