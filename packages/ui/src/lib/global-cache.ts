import { estimateRetainedBytes } from "./session-memory-budget"
...
function estimateCacheBytes(value: unknown): number {
  return estimateRetainedBytes(value, MAX_CACHE_ENTRY_BYTES)
}

function estimateCacheKeyBytes(params: CacheEntryParams): number {
  return [params.instanceId, params.sessionId, params.scope, params.cacheId, params.version]
    .reduce<number>((total, value) => total + (value?.length ?? 0) * 2 + 16, 0)
}

function recalculateRetainedBytes(): void {
  retainedBytes = 0
  retainedEntries = 0
...
export function setCacheEntry<T>(params: CacheEntryParams, value: T | undefined): void {
  ...
  retainedBytes -= existing?.byteSize ?? 0
  if (existing) retainedEntries -= 1
  const byteSize = estimateCacheBytes(value) + estimateCacheKeyBytes(params)
