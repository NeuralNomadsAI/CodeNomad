...
import { SESSION_MESSAGE_WINDOW_LIMIT } from "./message-v2/message-window"
...
export async function estimateNativeContentDeltaRetainedBytes(instanceId: string, sessionId: string, signal?: AbortSignal): Promise<number> {
  const state = streamingState.get(instanceId)?.get(sessionId)
  return state ? estimateRetainedBytesIncrementally(state, { signal, maxBytes: 64 * 1024 * 1024, maxNodes: 500_000 }) : 0
}
