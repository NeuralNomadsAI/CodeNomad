...
fn validate_main_window(window: &WebviewWindow, app_state: &AppState) -> Result<(), String> {
    ...
    if is_allowed_client_state_origin(&current_url, status.url.as_deref()) {
        Ok(())
    } else {
...
fn validate_access(
    ...
    validate_main_window(window, app_state)?;
    state.validate_renderer_access(access_token)
}
...
pub fn client_state_claim_access(
    ...
    validate_main_window(&window, &app_state)?;
    state.claim_renderer_access(&access_token)
}
...
pub fn client_state_load(
    ...
    validate_access(&window, &app_state, &state, &access_token)?;
    state.load()
}
