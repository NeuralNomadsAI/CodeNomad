...
test("finds a matching file after more than 8000 non-matching entries", async () => {
  ...
  const results = await searchWorkspaceFiles(workspace, "unique-search-target", {
    type: "file",
    refresh: true,
  })
  ...
  await searchWorkspaceFiles(workspace, "filler", { type: "file", refresh: true })
  assert.equal(getWorkspaceCandidates(workspace, "file\0filler")?.length, 8_000)
})

test("does not revisit directory links", async () => {
  ...
  try {
    ...
    assert.deepEqual(await searchWorkspaceFiles(cyclicWorkspace, "not-present", { refresh: true }), [])
  } finally {
    fs.rmSync(cyclicWorkspace, { recursive: true, force: true })
  }
})

test("indexes both real and linked directory paths", async () => {
  ...
  try {
    ...
    const linkedResults = await searchWorkspaceFiles(linkedWorkspace, "a/needle", { type: "file", refresh: true })
    const realResults = await searchWorkspaceFiles(linkedWorkspace, "b/needle", { type: "file", refresh: true })
