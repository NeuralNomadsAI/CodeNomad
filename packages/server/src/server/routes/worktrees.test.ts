...
test("a second server cannot delete a managed worktree leased by another manager", async (context) => {
  const temp = mkdtempSync(path.join(tmpdir(), "codenomad-cross-server-worktree-"))
  const git = (...args: string[]) => spawnSync("git", args, { cwd: temp, encoding: "utf8" })
  if (git("--version").error) {
    context.skip("Git is unavailable")
    rmSync(temp, { recursive: true, force: true })
  ...
  try {
    ...
    const manager = () => {
      const workspaceManager = new WorkspaceManager({
        ...
        binaryResolver: { resolve: () => ({ path: process.execPath, label: "Node.js" }) } as never,
        eventBus: new EventBus(),
        logger: pino({ level: "silent" }),
        getServerBaseUrl: () => "http://127.0.0.1:4000",
        workspaceLeaseDir: leaseDir,
        runtime: {
          launch: async () => ({ pid: process.pid, port: 4321, exitPromise: new Promise<never>(() => undefined), getLastOutput: () => "" }),
          stop: async () => undefined,
        },
      })
      ;(workspaceManager as any).waitForWorkspaceReadiness = async () => undefined
      return workspaceManager
    }
    const owner = manager(), deletingServer = manager()
    await owner.create(worktree.directory)

    const app = Fastify({ logger: false })
    registerWorktreeRoutes(app, {
      workspaceManager: {
        get: () => ({ id: "workspace", path: temp }),
        withWorkspacePathLease: deletingServer.withWorkspacePathLease.bind(deletingServer),
      } as never,
      sessionMetadataPersistence: {} as never,
      workflowManager: {
        withWorktreeOwnershipLease: async (_source: unknown, _worktree: unknown, operation: (owned: boolean) => Promise<unknown>) => operation(false),
    ...
    const blocked = await app.inject({ method: "DELETE", url: "/api/workspaces/workspace/worktrees/review" })
    assert.equal(blocked.statusCode, 409)
    assert.equal(existsSync(worktree.directory), true)

    await owner.shutdown()
    const deleted = await app.inject({ method: "DELETE", url: "/api/workspaces/workspace/worktrees/review" })
    assert.equal(deleted.statusCode, 204)
    assert.equal(existsSync(worktree.directory), false)
    await app.close()
  } finally {
