...
for (const method of ["outline", "window"]) test(`${method} validates ownership and fences location, project, revert and inventory changes`, async () => {
  ...
  try {
    ...
    f.ownership(true)
    assert.equal((await send()).json().status, method)
