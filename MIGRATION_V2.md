...
- Reduce every native streaming delta in order while keeping intermediate chunks off the renderer path; project accumulated text, reasoning, tool input, and compaction content at the next shared boundary, then refresh terminal events immediately.
...
| Compaction | Native checkpoint compaction summarizes the older head and retains a server-selected recent tail controlled by `compaction.keep.tokens`. | Uses `session.compact`; there is no message-level selective compaction. CodeNomad displays the accumulated partial summary through the shared streaming projection and replaces it with the terminal summary. | Add scoped controls only if V2 defines scoped compaction semantics. |
